...
<head>
  ...
  <title>Facility Reserve | LULAN</title>
      ...
      <div class="page-content">
        ...
        <div class="row">
          <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
              <div class="card-body">
                <button class="btn btn-primary mb-3"><a href="../functions/reserve-facility.php" class="text-light">Add Reservation</a></button>
                <div class="table-responsive">
                  <table id="dataTableExample" class="table table-hover">
                    ...
                    <tbody>
                      ...
                      $sql = "SELECT * FROM admin_facility_reservations JOIN admin_reserve_purpose ON admin_facility_reservations.rsrvPurpose_ID = admin_reserve_purpose.rsrvPurpose_ID JOIN admin_facility_info ON admin_facility_reservations.facility_ID = admin_facility_info.facility_ID";
                      $result = mysqli_query($con, $sql);
                      if ($result) {
                        while ($row = mysqli_fetch_assoc($result)) {
                          $rsrv_id = $row['rsrv_id'];
                          $rsrv_name = $row['rsrv_name'];
                          $facility_name = $row['facility_name'];
                          $rsrvPurpose_desc = $row['rsrvPurpose_desc'];
                          $rsrv_date = date('Y-m-d', strtotime($row['rsrv_date']));
                          $rsrv_time = date('g:i A', strtotime($row['rsrv_time']));
                          $rsrv_occupants = $row['rsrv_occupants'];
                          $rsrv_mobile = $row['rsrv_mobile'];
                          echo
                          '<tr>
        <th scope="row">' . $rsrv_id . '</th>
        <td>' . $rsrv_name . '</td>
        <td>' . $facility_name . '</td>
        <td><span class="badge bg-primary">' . $rsrvPurpose_desc . '</span></td>
        <td><span class="badge bg-secondary">' . $rsrv_date . '</span></td>
        <td><span class="badge bg-secondary">' . $rsrv_time . '</span></td>
        <td>' . $rsrv_occupants . '</td>
        <td>' . $rsrv_mobile . '</td>
        <td>
        <button class="btn btn-success btn-icon"><a href="../functions/view-facilityreserve.php?viewid=' . $rsrv_id . '" class="text-light"><i data-feather="eye"></i></a></button>
        <button class="btn btn-primary btn-icon"><a href="../functions/edit-facilityreserve.php?editid=' . $rsrv_id . '" class="text-light"><i data-feather="edit"></i></a></button>
        <button class="btn btn-warning btn-icon"><a href="../functions/delete-facilityreserve.php?deleteid=' . $rsrv_id . '" class="text-light"><i data-feather="archive"></i></a></button>
        </td>
    </tr>';
                        }
                      }
