...
<head>
    ...
    <title>Document Archive | LULAN</title>
                ...
                <div class="card text-center">
                    <div class="card-header">
                        <ul class="nav nav-tabs card-header-tabs">
                            <li class="nav-item"><a class="nav-link active" href="#">Archive</a></li>
                                        ...
                                        <div class="table-responsive">
                                            <table id="dataTableExample" class="table table-hover">
                                                ...
                                                <tbody>
                                                    ...
                                                    $sql = "SELECT * FROM admin_document_archive JOIN admin_document_format ON admin_document_archive.docu_fileFormat_id = admin_document_format.docu_fileFormat_id
        JOIN admin_document_category ON admin_document_archive.docu_categoryID = admin_document_category.docu_categoryID";
                                                    $result = mysqli_query($con, $sql);
                                                    if ($result) {
                                                        while ($row = mysqli_fetch_assoc($result)) {
                                                            $docu_id = $row['docu_id'];
                                                            $docu_title = $row['docu_title'];
                                                            $docu_fileFormat_desc = $row['docu_fileFormat_desc'];
                                                            $docu_categoryName = $row['docu_categoryName'];
                                                            $docu_path = $row['docu_path'];
                                                            $docu_status = $row['docu_status'];


                                                            echo '
            <tr>
                <td>' . uniqid($docu_id) . '</td>
                <td>' . $docu_title . '</td>
                <td><span class="badge bg-primary">' . $docu_fileFormat_desc . '</span></td>
                <td><span class="badge bg-primary">' . $docu_categoryName . '</span></td>
                <td>' . $docu_path . '</td>
                <td><span class="badge bg-warning">' . $docu_status . '</span></td>
                <td>
                <form method="post" action="../functions/retrieve-document.php">
            <input type="hidden" name="docu_id" value="' . $docu_id . '">
            <button type="submit" class="btn btn-success btn-icon-text text-light">
              <i class="btn-icon-prepend" data-feather="archive"></i>Retrieval
            </button>
          </form><br>
          <form method="post" action="../functions/dispose-document.php">
          <input type="hidden" name="docu_id" value="' . $docu_id . '">
          <button type="submit" class="btn btn-danger btn-icon-text text-light">
            <i class="btn-icon-prepend" data-feather="trash-2"></i>Disposal
          </button>
        </form>
          </td>
            </tr>';
