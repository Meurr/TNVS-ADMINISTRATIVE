...
<head>
  ...
  <title>Legal Contracts | LULAN</title>
      ...
      <div class="page-content">
        ...
        <div class="row">
          <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
              <div class="card-body">
                <button class="btn btn-primary mb-3"><a href="../functions/add-legalcontract.php" class="text-light">Add Contract</a></button>
                  ...
                  <table id="dataTableExample" class="table table-hover">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">Contract Author</th>
                        <th scope="col">Document ID</th>
                        <th scope="col">Contract Date</th>
                        <th scope="col">Contract Department</th>
                    ...
                    <tbody>
                      ...
                      $sql = "SELECT * FROM admin_legal_contract JOIN admin_document_info ON admin_legal_contract.docu_id = admin_document_info.docu_id
                                                                 JOIN admin_legal_contractstatus ON admin_legal_contract.contracts_status_id = admin_legal_contractstatus.contract_status_ID";
                      $result = mysqli_query($con, $sql);
                      if ($result) {
                        while ($row = mysqli_fetch_assoc($result)) {
                          $contracts_id = $row['contracts_id'];
                          $contracts_title = $row['contracts_title'];
                          $contracts_author = $row['contracts_author'];
                          $docu_title = $row['docu_title'];
                          $contracts_date = $row['contracts_date'];
                          $contracts_department = $row['contracts_department'];
                          $contract_status_name = $row['contract_status_name'];

                          // Set the badge color based on the approval status
                          switch ($contract_status_name) {
                            case 'approved':
                              $badge_color = 'success';
                              break;
                            case 'rejected':
                          ...
                          echo
                          '<tr>
        <th scope="row">' . $contracts_id . '</th>
        <td>' . $contracts_title . '</td>
        <td>' . $contracts_author . '</td>
        <td><span class="badge bg-primary">' . $docu_title . '</span></td>
        <td><span class="badge bg-secondary">' . $contracts_date . '</span></td>
        <td>' . $contracts_department . '</td>
        <td><span class="badge bg-' . $badge_color . '">' . $contract_status_name . '</span></td>
    </tr>';
                        }
                      }
