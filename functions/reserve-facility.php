...
if (isset($_POST['submit'])) {
  ...
  // Query database to get capacity and current status of the selected facility
  $query = "SELECT facility_capacity, facilityStatus_ID FROM admin_facility_info WHERE facility_ID = '$facility_ID'";
  $result = mysqli_query($con, $query);
  $row = mysqli_fetch_assoc($result);
  $facility_capacity = $row['facility_capacity'];
  $facility_status = $row['facilityStatus_ID'];

  // Check if the number of occupants exceeds the capacity of the facility
  if ($rsrv_occupants > $facility_capacity) {
    // Display an error message to the user and prevent the reservation from being made if the occupants is greater than capacity
    ...
    if ($result) {
      // Update facility status based on number of occupants
      if ($rsrv_occupants == 0) {
        // If no one is occupying the facility, set the status to available
        $facility_status = 1; // Available
      } elseif ($rsrv_occupants < $facility_capacity) {
        // If there are occupants but the capacity is not yet full, set the status to reserved
        $facility_status = 2; // Reserved
      } else {
        // If the number of occupants is equal to the capacity, set the status to occupied
        $facility_status = 3; // Occupied
      }

      // Update the facility status in the database
      $update_query = "UPDATE admin_facility_info SET facilityStatus_ID = '$facility_status' WHERE facility_ID = '$facility_ID'";
          ...
          <div class="card">
            <div class="row">
              ...
              <div class="auth-form-wrapper px-4 py-5">
                ...
                <h5 class="text-muted fw-normal mb-4">Add Reservation.</h5>
                <form id="signupForm" action="" method="POST">
                  ...
                  <div class="mb-3">
                    ...
                    <select class="form-select" name="facility_ID" required>
                      ...
                      $query = "SELECT facility_ID, facility_name FROM admin_facility_info";
                      $result = mysqli_query($con, $query);
                  ...
                  <div>
                    ...
                    <button class="btn btn-secondary text-white me-2 mb-2 mb-md-0"><a href="../modules/facility-reserve.php" class="text-light">Cancel</a></button>
                  </div>
                </form>
              </div>
            </div>
