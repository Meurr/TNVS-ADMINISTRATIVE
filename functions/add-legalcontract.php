...
if (isset($_POST['submit'])) {
    $contracts_title = $_POST['contracts_title'];
    $contracts_author = $_POST['contracts_author'];
    $docu_id = $_POST['docu_id'];
    $contracts_date = $_POST['contracts_date'];
    $contracts_department = $_POST['contracts_department'];
    $contracts_status_id = $_POST['contracts_status_id'];

    $sql = "INSERT INTO admin_legal_contract (contracts_title, contracts_author, docu_id, contracts_date, contracts_department, contracts_status_id)
    VALUES ('$contracts_title', '$contracts_author', '$docu_id', '$contracts_date', '$contracts_department', '$contracts_status_id')";

    $result = mysqli_query($con, $sql);
    if ($result) {
        // echo "Data Inserted Successfully";
        header('location:../modules/legal-contracts.php');
    } else {
                    ...
                    <div class="card">
                        <div class="row">
                            ...
                            <div class="auth-form-wrapper px-4 py-5">
                                ...
                                <h5 class="text-muted fw-normal mb-4">Add Contract.</h5>
                                <form id="signupForm" action="" method="POST">
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Contract Title</label>
                                        <input type="text" class="form-control" placeholder="Contract Title" name="contracts_title" autocomplete="off" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Contract Author</label>
                                        <input type="text" class="form-control" placeholder="Contracts Author" name="contracts_author" autocomplete="off" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="text" class="form-label">Document ID</label>
                                        <select class="form-select" name="docu_id" required>
                                            <option value="">Select Status</option>
                                            <?php
                                            // Retrieve the list of facilities from the database
                                            $query = "SELECT docu_id, docu_title FROM admin_document_info";
                                            $result = mysqli_query($con, $query);
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                // Display each facility as an option in the select element
                                                echo "<option value='" . $row['docu_id'] . "'>" . $row['docu_title'] . "</option>";
                                            }
                                            ?>
                                        </select>
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control" placeholder="Contract Date" name="contracts_date" autocomplete="off" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Contract Department</label>
                                        <input type="text" class="form-control" placeholder="Contract Department" name="contracts_department" autocomplete="off" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="text" class="form-label">Status</label>
                                        <select class="form-select" name="contracts_status_id" required>
                                            <option value="">Select Status</option>
                                            <?php
                                            // Retrieve the list of facilities from the database
                                            $query = "SELECT contract_status_ID, contract_status_name FROM admin_legal_contractstatus";
                                            $result = mysqli_query($con, $query);
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                // Display each facility as an option in the select element
                                                echo "<option value='" . $row['contract_status_ID'] . "'>" . $row['contract_status_name'] . "</option>";
                                            }
                                            ?>
                                        </select>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <button class="btn btn-secondary text-white me-2 mb-2 mb-md-0"><a href="../modules/legal-contracts.php" class="text-light">Cancel</a></button>
                                    </div>
                                </form>
                            </div>
                        </div>
